...
double prims_trials_lack (int trials, int dim, int v_count, point_4d *v_array, bool test);
...
//   srandom(time(NULL));
  if (atoi(argv[1]) != 0){
    ...
    if (atoi(argv[4]) != 0){
        ...
        sum_array = prims_trials_lack(atoi(argv[3]), atoi(argv[4]), v_count, v_array, true);
        free(v_array);
    } else {
        double *adj[v_count];
